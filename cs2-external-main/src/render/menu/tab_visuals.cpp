...
#include "features/footsteps_esp/footsteps_esp_ui.h"
#include <imgui.h>
...
void RenderTabVisuals() {
    ...
    if (UI::BeginCard("Player ESP")) {
        ...
        if (Config::Settings.esp.showBones) {
            ...
            ImGui::SliderFloat("Skeleton Max Dist", &Config::Settings.esp.skeletonMaxDistance, 0.0f, 100.0f, "%.0fm");
        }

        ImGui::Spacing();
        ImGui::SeparatorText("Frustum Culling");
        UI::SettingToggle("Show Off-screen Arrows", &Config::Settings.esp.showOffscreen);
