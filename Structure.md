...
*   **`process/`**: Управление процессом игры.
    ...
    *   `stealth.h/cpp`: Методы анти-детекта (в будущем).
...
*   **`sdk/`**: Техническая информация о CS2.
    ...
    *   `entity.h`: Определения классов игроков, костей (bones) и других объектов.

### 2. `src/features/` — Игровой функционал
Логика каждой конкретной функции чита. Все фичи наследуются от базового интерфейса `IFeature`.
...
*   **`renderer/`**: Инициализация графического движка (DirectX 11) и ImGui.
*   **`menu/`**: Дизайн и логика главного меню (настройка функций).
*   **`draw/`**: `DrawList` — промежуточный слой для удобного рисования линий, текста и фигур.

### 4. Дополнительные абстракции
...
*   **`input/`**: Обработка нажатий клавиш и централизованная генерация движений мыши (`InputManager::SendMouseDelta`).
*   **`external/`**: Сторонние библиотеки (ImGui, JSON, etc.).
